...
Curated source를 수집하고, 로컬 상태를 기준으로 우선순위를 정하고, 한국어 digest를 만들고, 필요할 때만 LLM을 설명 레이어로 붙입니다. Discord에서는 보이는 coordinator 하나를 front door로 두고, 그 뒤에 coding/research lane을 둡니다.
...
5. `#tech-brief`와 `#finance-brief`용 cron을 설치합니다.
